...
i=0
files = glob.glob(args.images_path+'results/camshift/*')
for f in files:
    os.remove(f)
...
for filename in sorted(glob.glob(args.images_path+"*.jpg")):
    ...
    if i==0:
        ...
        mask = cv.inRange(hsv_roi, 0, 100.)
        #______________________________
        #Other
        #hsv_roi = cv.cvtColor(roi, cv.COLOR_BGR2HSV)
        #mask = cv.inRange(hsv_roi, np.array((0., 0., 0.)), np.array((180., 255., 255.)))
        #______________________________
        roi_hist = cv.calcHist([hsv_roi], [0], mask, [100], [0, 180])
        ...
        cv.imshow('img2', img2)
        cv.imwrite(args.images_path + 'results/camshift/' + str(number) + '.jpg', img2)
        i+=1
        # time.sleep(0.1)
